...
<body class="bg-gray-50">
    <div class="min-h-screen">
        ...
        <main class="max-w-4xl mx-auto py-6 sm:px-6 lg:px-8">
            <div class="px-4 py-6 sm:px-0" x-data="contentItemReview()">
                <!-- Content Card -->
                <div class="bg-white rounded-lg shadow-lg overflow-hidden mb-6">
                    <!-- Concept Header -->
                    <div class="bg-gray-50 px-6 py-4 border-b">
                        <h2 class="text-xl font-semibold text-gray-900">{{ $contentItem->concept->title }}</h2>
                        <p class="text-sm text-gray-600 mt-1">{{ $contentItem->concept->notes }}</p>
                        <div class="flex items-center space-x-4 mt-3">
                            <span class="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                                {{ $contentItem->concept->status === 'Draft' ? 'bg-gray-100 text-gray-800' : '' }}
                                {{ $contentItem->concept->status === 'In Review' ? 'bg-yellow-100 text-yellow-800' : '' }}
                                {{ $contentItem->concept->status === 'Approved' ? 'bg-green-100 text-green-800' : '' }}
                                {{ $contentItem->concept->status === 'Scheduled' ? 'bg-blue-100 text-blue-800' : '' }}">
                                Concept: {{ $contentItem->concept->status }}
                            </span>
                            <span class="text-xs text-gray-500">
                                Due: {{ $contentItem->concept->due_date ? $contentItem->concept->due_date->format('M j, Y') : 'Not set' }}
                            </span>
                        </div>
                    </div>
                    ...
                    <div class="p-6">
                        <div class="flex items-start justify-between mb-6">
                            <div class="flex items-center space-x-3">
                                <span class="inline-flex items-center justify-center w-10 h-10 rounded-full text-lg {{ App\Services\ContentCalendarService::getPlatformColor($contentItem->platform) }}">
                                    {{ App\Services\ContentCalendarService::getPlatformIcon($contentItem->platform) }}
                                </span>
                                <div>
                                    <h3 class="text-lg font-medium text-gray-900 capitalize">{{ $contentItem->platform }} Post</h3>
                                    <p class="text-sm text-gray-500">
                                        Scheduled: {{ \Carbon\Carbon::parse($contentItem->scheduled_at)->format('M j, Y @ g:i A') }}
                                    </p>
                                </div>
                            </div>
                            <span class="inline-flex items-center px-3 py-1 rounded-full text-sm font-medium 
                                {{ $contentItem->status === 'Draft' ? 'bg-gray-100 text-gray-800' : '' }}
                                {{ $contentItem->status === 'In Review' ? 'bg-yellow-100 text-yellow-800' : '' }}
                                {{ $contentItem->status === 'Approved' ? 'bg-green-100 text-green-800' : '' }}
                                {{ $contentItem->status === 'Scheduled' ? 'bg-blue-100 text-blue-800' : '' }}">
                                {{ $contentItem->status }}
                            </span>
                        </div>
                        ...
                        <div class="bg-gray-50 border rounded-lg p-4 mb-6">
                            ...
                            <p class="text-gray-900 whitespace-pre-wrap">{{ $contentItem->copy }}</p>
                            @if($contentItem->media_url)
                                <div class="mt-3">
                                    <img src="{{ $contentItem->media_url }}" alt="Content media" class="max-w-md rounded-lg">
                                </div>
                            @endif
                        </div>

                        <!-- Action Buttons -->
                        @if($contentItem->status === 'In Review')
                        ...
                        <div class="border-t pt-6">
                            <h4 class="text-lg font-medium text-gray-900 mb-4">
                                Comments & Feedback
                                <span class="text-sm font-normal text-gray-500">({{ $contentItem->comments->count() }})</span>
                            </h4>
                            ...
                            <div class="space-y-4 mb-6">
                                ...
                                @if($contentItem->comments->isEmpty())
        ...
        function contentItemReview() {
            return {
                ...
                async approveVariant() {
                    if (confirm('Are you sure you want to approve this content?')) {
                        try {
                            ...
                            // In real implementation:
                            // await fetch('/api/content-items/approve', { ... })
                            // window.location.reload();
                ...
                async rejectVariant() {
                    ...
                    try {
                        // In a real app, this would make an API call
                        console.log('Rejecting content item with reason:', this.rejectReason);
                        ...
                        // In real implementation:
                        // await fetch('/api/content-items/reject', { ... })
                        // window.location.reload();
